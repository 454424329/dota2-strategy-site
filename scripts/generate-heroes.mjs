// Generate hero mock data from OpenDota API response
...
const chineseNames = {
  ...
  npc_dota_hero_kez: "凯",
  npc_dota_hero_largo: "拉果",
};

const heroes = JSON.parse(readFileSync(join(__dirname, "heroes_api.json"), "utf-8"));

const lines = [];
lines.push('import type { HeroData } from "@/types/dota";');
lines.push("");
lines.push("export const MOCK_HEROES: HeroData[] = [");

for (const h of heroes) {
  const zh = chineseNames[h.name] || h.localized_name;
  const attr = h.primary_attr === "all" ? "uni" : h.primary_attr;
  const attackType = h.attack_type === "Melee" ? "Melee" : "Ranged";
  const imgName = h.name.replace("npc_dota_hero_", "");

  lines.push(`  {`);
  lines.push(`    id: ${h.id},`);
  lines.push(`    name: "${h.name}",`);
  lines.push(`    localizedNameZh: "${zh.replace(/"/g, '\\"')}",`);
  lines.push(`    localizedNameEn: "${h.localized_name.replace(/"/g, '\\"')}",`);
  lines.push(`    primaryAttribute: "${attr}",`);
  lines.push(`    attackType: "${attackType}",`);
  lines.push(`    roles: ${JSON.stringify(h.roles)},`);
  lines.push(`    imageIcon: "${imgName}",`);
  lines.push(`    imagePortrait: "${imgName}_vert",`);
  lines.push(`    versionIntroduced: null,`);
...
const outputPath = join(__dirname, "..", "src", "lib", "data", "heroes-mock.ts");
writeFileSync(outputPath, lines.join("\n"), "utf-8");
console.log(`Written ${heroes.length} heroes to ${outputPath}`);
